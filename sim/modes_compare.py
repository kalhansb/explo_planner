#!/usr/bin/env python3
# Moved comments: docs/sim_notes/modes_compare_notes.md
...
# Cell-directory name to (arm, seed). The arm is everything from the first
# underscore to _seed<digits> at the end, so suffixed arms that run_campaign.sh
# requires (hybrid_seek) parse whole; do not narrow it to letters.
# (notes: modes-cell-name-regex)
RE_CELL = re.compile(r"_(.+)_seed(\d+)$")

# Reconnect-manoeuvre states. RETURN_HOME is excluded: it is the arm-invariant
# drive home, also in the control arm, and neither clause of cur in firings may
# count it (its rows carry reconnect_elapsed_sec = -1).
# (notes: modes-manoeuvre-states)
...
def gap_trace(a, b, start=200.0):
    ...
    # An unmeasurable end gap stays None; do not default it to 0.0, which would
    # report perfect agreement. (notes: modes-gap-end-none)
    end = g(t_end)
...
def measure(run_dir, thresh):
    ...
    if len(paths) != 2:
        # Fewer than two CSVs is an infrastructure failure, excluded by name.
        # Two is deliberate: gap_trace is pairwise and has no single value at
        # N>=3; event_log.py is the reader for larger teams.
        # (notes: modes-pairwise-csv-count)
    ...
    return dict(
        ...
        # Read at t_team, not end of run: the DONE drain and teardown tail after
        # finishing differ by arm. Censored runs have no t_team and fall back to
        # end of run. (notes: modes-dist-team-at-t-team)
...
# Above this many relabellings the test samples instead of enumerating; 30 v 30
# cannot be enumerated. 200000 keeps equal groups up to 10 v 10 exact, so
# floor-limited small comparisons stay exact. (notes: modes-perm-max-exact)
...
# Fixed and deliberately not a flag: the same cells must reproduce the same
# sampled p exactly, so a changed p can only mean changed data.
# (notes: modes-perm-seed-fixed)
PERM_SEED = 20260830

# Memo for perm_all keyed on the values: perm_p and perm_floor run back-to-back
# on the same lists for every row, so each row computes one distribution. Valid
# because it depends on the data alone. (notes: modes-perm-cache)
_PERM_CACHE = {}
...
def ladder(run_dirs, primary, spec):
    ...
    for th in ths:
        ...
        for d in sorted(run_dirs):
            ...
            if not m:
                # Unparseable directories are collected and named, never skipped
                # silently, so the ladder cannot disagree with the main table by
                # seeing fewer runs. (notes: modes-ladder-unparsed-cells)
    ...
    # A flip among thresholds near the primary means the endpoint is noise. A
    # flip between early and late thresholds is a result: a manoeuvre can cost
    # time early and repay it near completion.
    # (notes: modes-ladder-near-vs-early-flips)
...
def main():
    ...
    # Must match the planner's own done rule (unknown <= 0.64), since t_team is
    # the CSV crossing of it; a lower value censors most runs and withholds
    # t_team. The ladder brackets it on both sides.
    # (notes: modes-threshold-matches-done-rule)
    ...
    for d in sorted(args.runs):
        ...
        m = RE_CELL.search(os.path.basename(os.path.normpath(d)))
        if not m:
            # Unparseable directories are named, never skipped silently: a cell
            # the regex cannot read must not vanish from the comparison.
            # (notes: modes-main-unparsed-cells)
    ...
    for arm in sorted(arms):
        ...
        # lag and unk can come out empty and st.median([]) raises, so m() prints
        # a padded -- instead of aborting the rest of the report.
        # (notes: modes-empty-median-lists)
        def m(xs, w, p):
    ...
    if len(builds) > 1:
        # A differing git_explo_planner hash is necessary but not sufficient for
        # confounding: a sim/*.py commit moves it without changing the binary.
        # Print the check to run, not a verdict. (notes: modes-build-hash-check)
    ...
    # Coverage guard. (1) A non-censored run must end at or below the threshold,
    # or it did not hold its crossing. (2) Arms are compared over non-censored
    # runs only; the table's median pools censored runs.
    # (notes: modes-coverage-guard)
    ...
    if args.control not in cov or len(cov) < 2:
        ...
        # The band is the mean within-arm spread of this campaign, not a fixed
        # number. Arms with a single non-censored run are still compared but add
        # no spread. (notes: modes-coverage-band-from-data)
    ...
    # Median lag as a share of median t_team: the part of the endpoint a
    # manoeuvre that fires only after exploring can reach.
    # (notes: modes-mechanism-window)
    ...
    if lags and tts:
        ...
        # The terminal-manoeuvre conclusion holds only when these cells show no
        # mid-run dispatches; with any, print the caveat instead.
        # (notes: modes-mechanism-window-midrun)
    ...
    # Arm column width is aw, from the longest arm token present, so long
    # suffixed arm names cannot run into the metric column.
    # (notes: modes-arm-column-width)
    ...
    for arm in sorted(a for a in summary if a != ctl):
        ...
        for key, label in (("tt", "t_team"), ("lag", "lag"), ("dist", "dist_team")):
            ...
            # With any censored run in either arm the t_team delta is withheld:
            # a median over the survivors can reverse the true ranking, and a
            # footnote cannot fix a wrong-signed number.
            # (notes: modes-withhold-censored-t-team)
            if key == "tt" and ncens:
            ...
            if not exact:
                # Mark sampled rows in the note: exact and sampled rows can
                # share one table, and their floor columns mean different
                # things. (notes: modes-mark-sampled-rows)
