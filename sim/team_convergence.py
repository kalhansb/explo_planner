#!/usr/bin/env python3
# Moved comments: docs/sim_notes/team_convergence_notes.md
...
def precondition_stages(robots, census, exch, outages, require_exchange=True):
    ...
    # Checked before anything is scored: a run whose link never dropped has no
    # heal to score and is refused (2), not failed (1); nothing in a
    # permanently connected run can fault the exchange.
    # (notes: conv-no-dropout-refusal)
    stage = []
    if outages is None:
...
def main():
    ...
    # --- the heal, per outage ----------------------------------------------
    # Per outage: agreed again within --heal-window of recovery? Agreement is
    # sought over the rest of the run, but failing needs --heal-window of
    # undisturbed link after the heal, else censored. Keep the scored == 0
    # guard below. (notes: conv-heal-window-censoring)
    ...
    for start, end, recovered in outages:
        ...
        if not after:
            notes.append(f"outage [{start:.0f}, {end:.0f}] healed after the "
                         ...
                         f"scored")
            continue
        # An outage with no paired census sample inside it (shorter than one
        # census period) is skipped, not scored; otherwise healed would take
        # the next agreeing tick as a heal nothing observed.
        # (notes: conv-empty-outage-skip)
        if not during:
    ...
    for m in fails:
        print(f"FAIL\tconvergence\t{m}")
    # The PASS sentence itself states how many outages were scored and that the
    # convergence is not attributed to the exchange, so a PASS line read alone
    # cannot overclaim. (notes: conv-verdict-wording)
