...
public:
  ExploPlannerNode()
    ...
    // EIG-only planner: per-voxel score is the SCovox Beta expected
    // information gain. (The multi-planner selection lives in
    // exploration_planner_comp_node.)
...
private:
  ...
  void doPlan() {
    ...
    // Build a selection order: sort candidates by utility descending. The
    // cost-grid reachability filter runs after the sort as one of the
    // candidate filters in the walk below.
  ...
  // Planner-type id for the diagnostic RobotIntent. This node is EIG-only
  // (0=eig in the comparison node's enum).
  uint8_t plannerTypeId() const { return 0; }
...
int main(int argc, char** argv) {
  ...
  auto node =
      std::make_shared<explo_planner::ExploPlannerNode>();
