...
| `rendezvous_interval_sec` | double | `300.0` | Spacing of the shared meeting lattice (s). The agreed triple names an absolute `t_meet`, and the legal meeting instants are `t_meet + k*interval` — so this is how long a robot that cannot make the nearest instant must wait for the next one, and it is also the standing cost a punctual robot pays when a teammate rolls. Independent of the handshake's own 30 s derive/snapshot cadence, which is deliberately much faster. The scheduler may raise the first interval above this when the drive itself is longer; it never lowers it. Must be `> 0` — the node throws on a non-positive value rather than collapsing every rung onto `t_meet`. |
| `rendezvous_max_lateness_sec` | double | `60.0` | How late a robot may arrive and still agree to a meeting instant (s). Inside this budget a robot commits to the nearest lattice instant; past it, it rolls to the next one. The clamp is exact at zero, which is what stops a team that can ALL make the nearest instant from forking across two rungs. Non-finite or negative falls back to `60` with a WARN — a NaN would propagate into the lattice arithmetic and a negative budget would demand robots arrive early. |
...
| `reconnect_midrun_max_wait_sec` | double | `240.0` | Barrier give-up for a **mid-run** attempt. On expiry the robot resumes exploring — a mid-run attempt must never end the run, because the map is not saturated when one fires. The sim harness ships `30`, on the grounds that standing still is the dominated option whenever there is exploring left to do; see `MIDRUN_MAX_WAIT` for what the shorter cap costs in attempt budget. |
